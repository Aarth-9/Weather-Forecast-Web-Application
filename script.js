...
function searchByCity(){
    ...
    var urlsearch= `http://api.openweathermap.org/data/2.5/weather?q=${place}&` + `appid=${"KEY"}`;
...
function weatherReport(data){
    ...
    var urlcast= `http://api.openweathermap.org/data/2.5/forecast?q=${data.name}&` + `appid=${"KEY"}`;
...
function dayForecast(forecast){
    ...
    for (let i = 6; i < forecast.list.length; i+=7) {
